...
import { Download, Info, ChevronDown, ChevronUp, HelpCircle } from 'lucide-react';
...
export function DomainDashboard({ operationCode, operationName }: DomainDashboardProps) {
  ...
  // Close panel when changing operations
  useEffect(() => {
    setSelectedGpo(null);
  }, [operationCode]);
  ...
  }, [activeTab, operationCode, serverPage, workstationPage, userPage, reviewPage]);

  const loadOverview = async () => {
    setLoading(true);
    setError(null);
    try {
      const overview = await getDomainOverview(operationCode);
  ...
  const loadServerGpos = async () => {
    try {
      const data = await getServerGpos(operationCode, serverPage);
      setServerGpos(data);
    } catch (err) {
  ...
  const loadWorkstationGpos = async () => {
    try {
      const data = await getWorkstationGpos(operationCode, workstationPage);
  ...
  const loadUserGpos = async () => {
    try {
      const data = await getUserGpos(operationCode, userPage);
      setUserGpos(data);
    } catch (err) {
  ...
  const loadReviewGpos = async () => {
    try {
      const data = await getReviewRequired(operationCode, reviewPage);
      setReviewGpos(data);
    } catch (err) {
  ...
  if (loading) {
    return <LoadingSpinner message={`Loading ${operationName}...`} />;
  }
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={loadOverview} className="px-4 py-2 bg-primary-500 text-white rounded hover:bg-primary-600">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Synopsis Banner */}
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 m-4 mb-0 text-sm text-green-800">
      ...
      <div className="flex-1 p-6 overflow-auto bg-white">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">{operationName}</h2>
          ...
          <button
            onClick={async () => {
              try {
                await downloadDomainExcel(operationCode);
              } catch (err) {
